...
from classifier import FaceDataset, Classifier, LmSelector
# from resnet import ResNetC, ResNetLMS
# from unet import UNetLMS
...
def train(cl_net, lm_net, train_dataloader, epochs, filename, checkpoint_frequency=checkpoint_frequency, val_dataloader=None):
    ...
    # checkpoint_cl = torch.load(lp("/home/godsom/guesswhat/models/classifier/gm20.pt", clone=False))
    ...
    for epoch in range(epochs):
        for i, batch in enumerate(train_dataloader):
            ...
            # comb_masks = torch.clamp(comb_masks, min=0, max=1)
            # print(masks.shape)
...
def test():
    ...
    lm_net = LmSelector(n_lm=N_KEYPOINT).cuda()
...
def train_save(checkpoint_frequency=checkpoint_frequency):
    training_loss, validation_loss = train(
        Classifier().cuda(),
        LmSelector(n_lm=N_KEYPOINT).cuda(),
