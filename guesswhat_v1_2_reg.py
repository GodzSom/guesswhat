...
from classifier import Regressor, FaceDataset, LmSelector
...
PATH_TO_MODELS = lp("/home/godsom/guesswhat/models/v1_2_reg",clone=False)
PATH_TO_OUTPUTS = lp("/home/godsom/guesswhat/outputs/v1_2_reg",clone=False)
...
def train(cl_net, lm_net, train_dataloader, epochs, filename, checkpoint_frequency=checkpoint_frequency, val_dataloader=None):
    ...
    # if os.path.exists(lp("/home/godsom/guesswhat/logs/v1_2_reg"+'/'+args.model_di_n,clone=False)):
        ...
        if not os.path.exists(lp("/home/godsom/guesswhat/logs/v1_2_reg"+'/'+args.model_di_n,clone=False)):
            os.makedirs(lp("/home/godsom/guesswhat/logs/v1_2_reg"+'/'+args.model_di_n,clone=False))
            os.makedirs(PATH_TO_MODELS+'/'+args.model_di_n)
        start_epoch = 0

    writer = SummaryWriter(lp("/home/godsom/guesswhat/logs/v1_2_reg/"+args.model_di_n,clone=False))

    checkpoint_cl = torch.load(lp("/home/godsom/guesswhat/models/regressor/gm20.pt",clone=False))
    ...
    for epoch in range(epochs):
        for i, batch in enumerate(train_dataloader):
            ...
            loss = torch.mean((outputs[:,0]-labels)**2) + variance_loss(probmap, lms)*10
            ...
            accu = 0#um(outputs == labels).item()/outputs.shape[0]
...
def validate(cl_net, lm_net, dataloader,epoch,inv_std_loop,start_epoch):
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            ...
            exact_match += 0#sum(outputs == labels).item()
            if i==0:
                if N_KEYPOINT ==2:
                    probmap = probmap.cpu()
                    masked_images = torch.transpose(masked_images, 1 ,2)
...
def test():
    ...
    cl_net = Regressor().cuda()
    lm_net = LmSelector(N_KEYPOINT).cuda()

    checkpoint = torch.load(PATH_TO_MODELS+'/'+args.model_di_n+f'/{args.model_di_n}.pt')
    # cl_net.load_state_dict(checkpoint['model_state_dict'])
    lm_net.load_state_dict(checkpoint['lm_state_dict'])

    checkpoint = torch.load("/home/godsom/guesswhat/models/regressor/gm20.pt")
    cl_net.load_state_dict(checkpoint['model_state_dict'])
        ...
        for i, batch in enumerate(get_dataloader(batch_size, 'test')):
            ...
            if i<50:
                if N_KEYPOINT ==2:
                    ...
                    # plt.subplot(2, 2, 4)
                    # plt.imshow(masked_images[1])
                    plt.savefig(PATH_TO_OUTPUTS+'/'+args.model_di_n+'/test/test_{:04d}.png'.format(epoch+start_epoch))
                    plt.close()
                else:
