...
from classifier import FaceDataset, Classifier#, Classifier, ResNetLMS
...
PATH_TO_MODELS = lp("/home/godsom/guesswhat/models/v3",clone=False)
PATH_TO_OUTPUTS = lp("/home/godsom/guesswhat/outputs/v3",clone=False)
...
def train(cl_net, lm_net, train_dataloader, epochs, filename, checkpoint_frequency=checkpoint_frequency, val_dataloader=None):
    ...
    # if os.path.exists(lp("/home/godsom/guesswhat/logs/v3"+'/'+args.model_di_n,clone=False)):
        ...
        if not os.path.exists(lp("/home/godsom/guesswhat/logs/v3"+'/'+args.model_di_n,clone=False)):
            os.makedirs(lp("/home/godsom/guesswhat/logs/v3"+'/'+args.model_di_n,clone=False))
            os.makedirs(PATH_TO_MODELS+'/'+args.model_di_n)
        start_epoch = 0

    writer = SummaryWriter(lp("/home/godsom/guesswhat/logs/v3/"+args.model_di_n,clone=False))

    checkpoint_cl = torch.load(lp("/home/godsom/guesswhat/models/classifier/gm20_n2.pt", clone=False))
    ...
    for epoch in range(epochs):
        for i, batch in enumerate(train_dataloader):
            ...
            comb_masks, probmap, lms, all_masks = lm_net(images)

            masked_images1 = images * comb_masks.repeat(1,3,1,1)
            # print(probmap.shape)
            masked_images2 = images * all_masks[:,0,:,:][:,None].repeat(1,3,1,1)
            masked_images = torch.cat([masked_images1, masked_images2], dim=0)
...
def test():
    ...
    lm_net = CLMS(n_lm=N_KEYPOINT).cuda()
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(get_dataloader(batch_size, 'test')):
            images, labels = batch['image'].cuda(), batch['label'].cuda()
            comb_masks, probmap, lms = lm_net(images)

            masked_images1 = images * comb_masks.repeat(1,3,1,1)
            masked_images2 = images * probmap[:,0,:,:][:,None].repeat(1,3,1,1)
            masked_images = torch.cat([masked_images1, masked_images2], dim=0)
            labels = torch.cat([labels, labels], dim=0)
