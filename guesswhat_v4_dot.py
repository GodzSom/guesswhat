...
from classifier import FaceDataset, Classifier, new_lms#, Classifier, new_lms
# from resnet import ResNetLMS_L
# from clms import new_lms
...
PATH_TO_MODELS = lp("/home/godsom/guesswhat/models/v4_dot",clone=False)
PATH_TO_OUTPUTS = lp("/home/godsom/guesswhat/outputs/v4_dot",clone=False)
...
def train(cl_net, lm_net, train_dataloader, epochs, filename, checkpoint_frequency=checkpoint_frequency, val_dataloader=None):
    ...
    # if os.path.exists(lp("/home/godsom/guesswhat/logs/v4_dot"+'/'+args.model_di_n,clone=False)):
        ...
        if not os.path.exists(lp("/home/godsom/guesswhat/logs/v4_dot"+'/'+args.model_di_n,clone=False)):
            os.makedirs(lp("/home/godsom/guesswhat/logs/v4_dot"+'/'+args.model_di_n,clone=False))
            os.makedirs(PATH_TO_MODELS+'/'+args.model_di_n)
        start_epoch = 0

    writer = SummaryWriter(lp("/home/godsom/guesswhat/logs/v4_dot/"+args.model_di_n,clone=False))
    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            loss = 10*criterion(outputs, labels) + 1*variance_loss(probmap, lms)# + dis_pel_loss(dist)
        ...
        if (epoch) % 1 == 0 and val_dataloader is not None:
            # if inv_std_loop < 20:
...
def test():
    ...
    lm_net = new_lms(n_lm=N_KEYPOINT).to(device)
