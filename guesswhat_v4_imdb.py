...
from classifier import IMDBDataset, Classifier#, LmSelector#, Classifier, ResNetLMS_L
...
file_dir =  lp('/home/godsom/Dataset/imdb/imdb_crop')
PATH_TO_MODELS = lp("/home/godsom/guesswhat/models/v4_imdb",clone=False)
PATH_TO_OUTPUTS = lp("/home/godsom/guesswhat/outputs/v4_imdb",clone=False)
...
mat = loadmat(lp('/home/godsom/Dataset/imdb/imdb_crop/imdb.mat'))

labels = (mat['imdb'][0][0][3][0])
notnan_id = np.argwhere(~np.isnan(labels))
# print(notnan_id)
labels = labels[notnan_id]
fl = (mat['imdb'][0][0][2][0])
fl = fl[notnan_id]
...
def get_dataloader(minibatch_size=batch_size, mode='train'):
    root_dir = file_dir
    if mode == 'train':
        transformed_dataset = IMDBDataset(root_dir, fl[:100000], labels[:100000], transforms.Compose(transforms_list))
        # transformed_dataset = IMDBDataset(root_dir, fl[:100], transforms.Compose(transforms_list))
    if mode == 'val':
        transformed_dataset = IMDBDataset(root_dir, fl[100000:100500], labels[100000:100500], transforms.Compose(transforms_list))
        # transformed_dataset = IMDBDataset(root_dir, fl[20000:20100], transforms.Compose(transforms_list))
    if mode == 'test':
        transformed_dataset = IMDBDataset(root_dir, fl[100500:101000], labels[100500:101000], transforms.Compose(transforms_list))
...
def train(cl_net, lm_net, train_dataloader, epochs, filename, checkpoint_frequency=checkpoint_frequency, val_dataloader=None):
    ...
    # if os.path.exists(lp("/home/godsom/guesswhat/logs/v4_imdb"+'/'+args.model_di_n,clone=False)):
        ...
        if not os.path.exists(lp("/home/godsom/guesswhat/logs/v4_imdb"+'/'+args.model_di_n,clone=False)):
            os.makedirs(lp("/home/godsom/guesswhat/logs/v4_imdb"+'/'+args.model_di_n,clone=False))
            os.makedirs(PATH_TO_MODELS+'/'+args.model_di_n)
        start_epoch = 0

    writer = SummaryWriter(lp("/home/godsom/guesswhat/logs/v4_imdb/"+args.model_di_n,clone=False))
    ...
    for epoch in range(epochs):
        for i, batch in enumerate(train_dataloader):
            ...
            if N_KEYPOINT==1:
                masked_images = images * tile(masks[:,0,:,:][:,None], 1, 3)
            elif N_KEYPOINT==2:
                masked_images1 = images * tile(masks[:,0,:,:][:,None], 1, 3)

                comb_masks2 = torch.max(masks[:,0,:,:], masks[:,1,:,:])
                masked_images2 = images * tile(comb_masks2[:,None], 1, 3)
...
def test():
    ...
    lm_net = ResNetLMS_L(n_lm=N_KEYPOINT).cuda()
...
def train_save(checkpoint_frequency=checkpoint_frequency):
    training_loss, validation_loss = train(
        Classifier().cuda(),
        ResNetLMS_L(n_lm=N_KEYPOINT).cuda(),
