...
PATH_TO_MODELS = lp("/home/godsom/guesswhat/models/v6_dot",clone=False)
PATH_TO_OUTPUTS = lp("/home/godsom/guesswhat/outputs/v6_dot",clone=False)
...
COORS = torch.reshape(COORS, (1,-1,2)).to(device)
# print(COORS)
# assert False
...
def get_dataloader(minibatch_size=batch_size, mode='train'):
    ...
    if mode == 'train':
        ...
        transformed_dataset = FaceDataset(root_dir, fl[:120], transforms.Compose(transforms_list))
    if mode == 'val':
        # transformed_dataset = FaceDataset(root_dir, fl[1800:2000], transforms.Compose(transforms_list))
        transformed_dataset = FaceDataset(root_dir, fl[120:180], transforms.Compose(transforms_list))
    if mode == 'test':
        transformed_dataset = FaceDataset(root_dir, fl[1800:2000], transforms.Compose(transforms_list))
...
def train(cl_net, lm_net, train_dataloader, epochs, filename, checkpoint_frequency=checkpoint_frequency, val_dataloader=None):
    ...
    # if os.path.exists(lp("/home/godsom/guesswhat/logs/v6_dot"+'/'+args.model_di_n,clone=False)):
        ...
        if not os.path.exists(lp("/home/godsom/guesswhat/logs/v6_dot"+'/'+args.model_di_n,clone=False)):
            os.makedirs(lp("/home/godsom/guesswhat/logs/v6_dot"+'/'+args.model_di_n,clone=False))
            os.makedirs(PATH_TO_MODELS+'/'+args.model_di_n)
        start_epoch = 0

    writer = SummaryWriter(lp("/home/godsom/guesswhat/logs/v6_dot/"+args.model_di_n,clone=False))
    ...
    for epoch in range(epochs):
        ...
        for aa in range(30):
            ...
            r5 = random.randint(40, 160)
            r6 = random.randint(40, 160)
            ...
            cr.arc(r5 , r6, 10, 0, 2*pi)
            cr.set_line_width(3)
            cr.set_source_rgb(1.0, 0.75, 0.8)
            cr.stroke()

            # write output
            surface.write_to_png("/home/godsom/Dataset/dot/{:03d}_{:01d}_0.png".format(aa,2))
            ####################################
            ...
            # write output
            surface.write_to_png("/home/godsom/Dataset/dot/{:03d}_{:01d}_0.png".format(aa,3))
            ...
            # write output
            surface.write_to_png("/home/godsom/Dataset/dot/{:03d}_{:01d}_0.png".format(aa,4))
            ...
            # write output
            surface.write_to_png("/home/godsom/Dataset/dot/{:03d}_{:01d}_0.png".format(aa,5))
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            patch_score = torch.reshape(patch_score, (batch_size, n_win_sqrt*n_win_sqrt, N_KEYPOINT))
            patch_score = torch.transpose(patch_score, 1, 2)
            patch_score = F.softmax(patch_score, dim=2)
            patch_score = torch.reshape(patch_score, (batch_size, N_KEYPOINT, n_win_sqrt*n_win_sqrt))
            patch_score = torch.transpose(patch_score, 0, 1)
            patch_score = torch.reshape(patch_score, (N_KEYPOINT, batch_size*n_win_sqrt*n_win_sqrt))

            patch_score_all = torch.reshape(patch_score, (N_KEYPOINT, batch_size, n_win_sqrt*n_win_sqrt))
            top_score, ts_idx = torch.topk(patch_score_all, n_win_sqrt*n_win_sqrt)

            # print(ts_idx.shape)

            paired_windows = torch.cat([windows[ts_idx[0,:,:].view(-1)], windows[ts_idx[1,:,:].view(-1)]], dim=1)

            outputs = cl_net(paired_windows)

            # print(labels)
            # print(tile(labels, 0, n_win_sqrt*n_win_sqrt))
            # print(patch_score.shape)
            tiled_labels = tile(labels, 0, n_win_sqrt*n_win_sqrt)
            patch_score_mean = 0.5*(patch_score[0][ts_idx[0,:,:].view(-1)] + patch_score[1][ts_idx[1,:,:].view(-1)])
            loss = torch.sum(criterion(outputs, tiled_labels) * (patch_score_mean)) #+ 1*variance_loss(probmap, lms)# + dis_pel_loss(dist)
        ...
        if (epoch+1) % checkpoint_frequency == 1 and val_dataloader is not None:
            print('--')
            print(ts_idx.shape)
            # if inv_std_loop < 20:
            #     inv_std_loop = inv_std_loop * anneal_rate

            # print(train_accu.shape)
            # print(ts_idx.shape)
            # print(train_accu[ts_idx[:,0]])
            ...
            # print(ts_idx)
            # print(train_accu[ts_idx[0,0,:]])
            # print(sum(train_accu[ts_idx[0,:,0]]).item())
            # print(sum(train_accu[ts_idx[1,:,0]]).item())
            ak_accu = 0
            for qq in range(batch_size):
                ak_accu = ak_accu + train_accu[qq*batch_size]
            ak_accu = ak_accu/batch_size
            print(ak_accu)
            print(f'epoch:{epoch+start_epoch}, training_error:{training_loss[-1]}, val_error:{validation_loss[-1]}, val_accu:{val_accuracy[-1]}')
            print(f'top accuracy, training:{ak_accu}, val:{val_results[2]}')
            ...
            if N_KEYPOINT ==2:
                sas = 0
                # patch_score_plt = torch.reshape(patch_score, (N_KEYPOINT, batch_size, n_win_sqrt*n_win_sqrt))[:,0]
                ...
                patch_score_plt = patch_score_all[:,1]
                top_score, ts_idx = torch.topk(patch_score_plt, 1)
                plt.figure(figsize=(12,12))
                # plt.subplot(1, 2, 1)
                plt.imshow(patches_plot[n_win_sqrt*n_win_sqrt+ts_idx[0][0]])
                # plt.subplot(1, 2, 2)
                # plt.imshow(patches_plot[ts_idx[1][0]])
                plt.savefig(PATH_TO_OUTPUTS+'/'+args.model_di_n+'/train/p_{:04d}_g.png'.format(epoch+start_epoch))
                plt.close()
            else:
...
def validate(cl_net, lm_net, dataloader,epoch,inv_std_loop,start_epoch):
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            ...
            patch_score_all = torch.reshape(patch_score, (N_KEYPOINT, batch_size, n_win_sqrt*n_win_sqrt))
            top_score_all, ts_idx_all = torch.topk(patch_score_all, n_win_sqrt*n_win_sqrt)

            paired_windows = torch.cat([windows[ts_idx_all[0,:,:].view(-1)], windows[ts_idx_all[1,:,:].view(-1)]], dim=1)

            outputs = cl_net(paired_windows)

            tiled_labels = tile(labels, 0, n_win_sqrt*n_win_sqrt)
            patch_score_mean = 0.5*(patch_score[0][ts_idx_all[0,:,:].view(-1)] + patch_score[1][ts_idx_all[1,:,:].view(-1)])
            loss = torch.sum(criterion(outputs, tiled_labels) * patch_score_mean) #+ 1*variance_loss(probmap, lms)# + dis_pel_loss(dist)
            ...
            if i==0:
                if N_KEYPOINT ==2:
                    ...
                    # print(top_score)
                    # # print(ts_idx)
                    patches_plot = torch.transpose(paired_windows, 1, 2)
                    patches_plot = torch.transpose(patches_plot, 2, 3)#.detach().cpu().numpy()

                    plt.figure(figsize=(12,12))
                    plt.subplot(1, 2, 1)
                    # plt.imshow(patches_plot[ts_idx[0,0]])
                    plt.imshow(patches_plot[0,:,:,:3].detach().cpu().numpy())
                    plt.subplot(1, 2, 2)
                    plt.imshow(patches_plot[0,:,:,3:].detach().cpu().numpy())
                    plt.savefig(PATH_TO_OUTPUTS+'/'+args.model_di_n+'/p_{:04d}_0.png'.format(epoch+start_epoch))
                    plt.close()
                    ...
                    plt.figure(figsize=(12,12))
                    plt.subplot(1, 2, 1)
                    plt.imshow(patches_plot[49,:,:,:3].detach().cpu().numpy())
                    plt.subplot(1, 2, 2)
                    plt.imshow(patches_plot[49,:,:,3:].detach().cpu().numpy())
                    plt.savefig(PATH_TO_OUTPUTS+'/'+args.model_di_n+'/p_{:04d}_1.png'.format(epoch+start_epoch))
                    plt.close()

                    # patch_score_plt = patch_score_all[:,2]
                    # top_score, ts_idx = torch.topk(patch_score_plt, 1)
                    # plt.figure(figsize=(12,12))
                    # plt.subplot(1, 2, 1)
                    # plt.imshow(patches_plot[ts_idx[0,0]])
                    # plt.subplot(1, 2, 2)
                    # plt.imshow(patches_plot[ts_idx[1,0]])
                    # plt.savefig(PATH_TO_OUTPUTS+'/'+args.model_di_n+'/p_{:04d}_2.png'.format(epoch+start_epoch))
                    # plt.close()
                    #
                    # patch_score_plt = patch_score_all[:,3]
                    # top_score, ts_idx = torch.topk(patch_score_plt, 1)
                    # plt.figure(figsize=(12,12))
                    # plt.subplot(1, 2, 1)
                    # plt.imshow(patches_plot[ts_idx[0,0]])
                    # plt.subplot(1, 2, 2)
                    # plt.imshow(patches_plot[ts_idx[1,0]])
                    # plt.savefig(PATH_TO_OUTPUTS+'/'+args.model_di_n+'/p_{:04d}_3.png'.format(epoch+start_epoch))
                    # plt.close()
                    #
                    # patch_score_plt = patch_score_all[:,4]
                    # top_score, ts_idx = torch.topk(patch_score_plt, 1)
                    # plt.figure(figsize=(12,12))
                    # plt.subplot(1, 2, 1)
                    # plt.imshow(patches_plot[ts_idx[0,0]])
                    # plt.subplot(1, 2, 2)
                    # plt.imshow(patches_plot[ts_idx[1,0]])
                    # plt.savefig(PATH_TO_OUTPUTS+'/'+args.model_di_n+'/p_{:04d}_4.png'.format(epoch+start_epoch))
                    # plt.close()
                    #
                    # patch_score_plt = patch_score_all[:,5]
                    # top_score, ts_idx = torch.topk(patch_score_plt, 1)
                    # plt.figure(figsize=(12,12))
                    # plt.subplot(1, 2, 1)
                    # plt.imshow(patches_plot[ts_idx[0,0]])
                    # plt.subplot(1, 2, 2)
                    # plt.imshow(patches_plot[ts_idx[1,0]])
                    # plt.savefig(PATH_TO_OUTPUTS+'/'+args.model_di_n+'/p_{:04d}_5.png'.format(epoch+start_epoch))
                    # plt.close()
                elif N_KEYPOINT ==4:
                    patches_plot = patches[0]
                    # print(patches_plot.shape)
                    patches_plot = torch.transpose(patches_plot, 0 ,1)
                    patches_plot = torch.transpose(patches_plot, 1, 2).detach().cpu().numpy()

                    plt.figure(figsize=(12,12))
                    plt.subplot(2, 2, 1)
                    plt.imshow(patches_plot[:,:,:3])
                    plt.subplot(2, 2, 2)
                    plt.imshow(patches_plot[:,:,3:6])
                    plt.subplot(2, 2, 3)
                    plt.imshow(patches_plot[:,:,6:9])
                    plt.subplot(2, 2, 4)
                    plt.imshow(patches_plot[:,:,9:12])
                    ...
                    # plt.subplot(1, 2, 2)
                    # plt.imshow(patches_plot[ts_idx[1][0]])
                    plt.savefig(PATH_TO_OUTPUTS+'/'+args.model_di_n+'/p_{:04d}_g.png'.format(epoch+start_epoch))
                    plt.close()
                else:
    ...
    ak_accu = 0
    for qq in range(batch_size):
        ak_accu = ak_accu + val_accu[qq*batch_size]
    ak_accu = ak_accu/batch_size
    # print(ak_accu)
    # print(ak_accu)
    return total_loss/i, exact_match/total, ak_accu

# def get_validation_error(c, fold, train_transform_index):
#     filename = get_model_filename(c, fold, train_transform_index)
#     net = Classifier_v5().to(device)
...
def test():
    ...
    cl_net = Classifier_v5().to(device)
    lm_net = LmSelector_v5(n_lm=N_KEYPOINT).to(device)
...
def train_save(checkpoint_frequency=checkpoint_frequency):
    training_loss, validation_loss = train(
        ...
        LmSelector_v5(n_lm=N_KEYPOINT).to(device),
