...
def get_dataloader(minibatch_size=batch_size, mode='train'):
    ...
    if mode == 'val':
        ...
        # transformed_dataset = FaceDataset(root_dir, fl[100:200], transforms.Compose(transforms_list))
    if mode == 'test':
        transformed_dataset = FaceDataset(root_dir, fl[1800:2000], transforms.Compose(transforms_list))
...
def train(cl_net, lm_net, train_dataloader, epochs, filename, checkpoint_frequency=checkpoint_frequency, val_dataloader=None):
    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            loss = torch.sum(criterion(outputs, tiled_labels) * patch_score) #+ 1*variance_loss(probmap, lms)# + dis_pel_loss(dist)
        ...
        if (epoch+1) % checkpoint_frequency == 1 and val_dataloader is not None:
            ...
            print(f'top accuracy, training:{sum(train_accu[ts_idx[0,:]]).item()/batch_size}, val:{val_results[2]}')
...
def validate(cl_net, lm_net, dataloader,epoch,inv_std_loop,start_epoch):
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            ...
            if i==0:
                if N_KEYPOINT ==2:
                    patch_score_plt = torch.reshape(patch_score, (bs, N_KEYPOINT, n_win_sqrt*n_win_sqrt))[0]
                    top_score, ts_idx = torch.topk(patch_score_plt, 1)
                    patches_plot = torch.transpose(windows, 1, 2)
                    patches_plot = torch.transpose(patches_plot, 2, 3).detach().cpu().numpy()
    ...
    return total_loss/i, exact_match/total, sum(val_accu[ts_idx_all[0,:]]).item()/batch_size
