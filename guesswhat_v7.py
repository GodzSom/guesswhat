...
from classifier import FaceDataset, Classifier#, LmSelector#, Classifier, ResNetLMS_L
...
PATH_TO_MODELS = lp("/home/godsom/guesswhat/models/v4",clone=False)
PATH_TO_OUTPUTS = lp("/home/godsom/guesswhat/outputs/v4",clone=False)
...
def train(cl_net, lm_net, train_dataloader, epochs, filename, checkpoint_frequency=checkpoint_frequency, val_dataloader=None):
    ...
    # if os.path.exists(lp("/home/godsom/guesswhat/logs/v4"+'/'+args.model_di_n,clone=False)):
        ...
        if not os.path.exists(lp("/home/godsom/guesswhat/logs/v4"+'/'+args.model_di_n,clone=False)):
            os.makedirs(lp("/home/godsom/guesswhat/logs/v4"+'/'+args.model_di_n,clone=False))
            os.makedirs(PATH_TO_MODELS+'/'+args.model_di_n)
        start_epoch = 0

    writer = SummaryWriter(lp("/home/godsom/guesswhat/logs/v4/"+args.model_di_n,clone=False))
    ...
    for epoch in range(epochs):
        for i, batch in enumerate(train_dataloader):
            ...
            if N_KEYPOINT==1:
                ...
                comb_masks2 = torch.max(masks[:,0,:,:], masks[:,1,:,:])
                masked_images2 = images * tile(comb_masks2[:,0,:,:][:,None], 1, 3)

                comb_masks3 = torch.max(comb_masks2, masks[:,2,:,:])
                masked_images3 = images * tile(comb_masks3[:,0,:,:][:,None], 1, 3)

                comb_masks4 = torch.max(comb_masks3, masks[:,3,:,:])
                masked_images4 = images * tile(comb_masks4[:,0,:,:][:,None], 1, 3)
