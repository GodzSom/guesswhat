...
# from classifier_v2 import LmSelector
...
PATH_TO_MODELS = lp("/home/godsom/guesswhat/models/v7_dot_6",clone=False)
PATH_TO_OUTPUTS = lp("/home/godsom/guesswhat/outputs/v7_dot_6",clone=False)
...
def get_transform_mat_rand(probmap, inv_std=inv_std):
    coors = (torch.rand(probmap.shape[0],N_KEYPOINT,2)*2) - 1
    ...
    for i in range(probmap.shape[0]):
        score[i] = probmap[i, 0, int(coors[i,0,0]), int(coors[i,0,1])] + probmap[i, 0, int(coors[i,1,0]), int(coors[i,1,1])]
...
def train(cl_net, lm_net, train_dataloader, epochs, filename, checkpoint_frequency=checkpoint_frequency, val_dataloader=None):
    ...
    # if os.path.exists(lp("/home/godsom/guesswhat/logs/v7_dot_6"+'/'+args.model_di_n,clone=False)):
        ...
        if not os.path.exists(lp("/home/godsom/guesswhat/logs/v7_dot_6"+'/'+args.model_di_n,clone=False)):
            os.makedirs(lp("/home/godsom/guesswhat/logs/v7_dot_6"+'/'+args.model_di_n,clone=False))
            os.makedirs(PATH_TO_MODELS+'/'+args.model_di_n)
        start_epoch = 0

    writer = SummaryWriter(lp("/home/godsom/guesswhat/logs/v7_dot_6/"+args.model_di_n,clone=False))
    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            if N_KEYPOINT==1:
                ...
                # masked_images1 = images * tile(masks[:,0,:,:][:,None], 1, 3)

                comb_masks2 = torch.max(masks[:,0,:,:], masks[:,1,:,:])
                masked_images = images * tile(comb_masks2[:,None], 1, 3)

                # masked_images = torch.cat([masked_images2, masked_images1], dim=0)
                # labels = torch.cat([labels, labels], dim=0)

                rand_comb_masks2 = torch.max(rand_masks[:,0,:,:], rand_masks[:,1,:,:])
                rand_masked_images = images * tile(rand_comb_masks2[:,None], 1, 3)
            elif N_KEYPOINT==4:
                comb_masks2 = torch.max(masks[:,0,:,:], masks[:,1,:,:])
                comb_masks2 = torch.max(comb_masks2, masks[:,2,:,:])
                comb_masks2 = torch.max(comb_masks2, masks[:,3,:,:])
                masked_images = images * tile(comb_masks2[:,None], 1, 3)

                # rand_comb_masks2 = torch.max(rand_masks[:,0,:,:], rand_masks[:,1,:,:])
                # rand_comb_masks2 = torch.max(rand_comb_masks2, rand_masks[:,2,:,:])
                # rand_comb_masks2 = torch.max(rand_comb_masks2, rand_masks[:,3,:,:])
                rand_masked_images = images #* tile(rand_comb_masks2[:,None], 1, 3)
            ...
            loss = torch.sum(criterion(outputs, labels)) + torch.sum(criterion(rand_outputs, labels)) + variance_loss(probmap, lms)
            # print(pm_score)
            ...
            rand_outputs = torch.argmax(rand_outputs, 1)
            accu = sum(outputs == labels).item()/outputs.shape[0]
            rand_accu = sum(rand_outputs == labels).item()/rand_outputs.shape[0]
        ...
        if (epoch) % 10 == 0 and val_dataloader is not None:
            ...
            if N_KEYPOINT ==2:
                ...
                rand_masked_images = torch.transpose(rand_masked_images, 1 ,2)
                rand_masked_images = torch.transpose(rand_masked_images, 2, 3)
                rand_masked_images = rand_masked_images.detach().cpu().numpy()
                ...
                plt.subplot(2, 2, 4)
                plt.imshow(rand_masked_images[0])
...
def validate(cl_net, lm_net, dataloader,epoch,inv_std_loop,start_epoch):
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            ...
            loss = torch.sum(criterion(outputs, labels)) #+ torch.sum(criterion(rand_outputs, labels))
            ...
            if i==0:
                if N_KEYPOINT ==2:
                    ...
                    plt.savefig(PATH_TO_OUTPUTS+'/'+args.model_di_n+'/val_A_{:04d}.png'.format(epoch+start_epoch))
                    plt.close()
                    ...
                    plt.figure(figsize=(12,12))
                    plt.subplot(2, 2, 1)
                    plt.imshow(probmap[5][0]/torch.max(probmap[5][0]))
                    plt.subplot(2, 2, 2)
                    plt.imshow(probmap[5][1]/torch.max(probmap[5][1]))
                    plt.subplot(2, 2, 3)
                    plt.imshow(probmap[5][2]/torch.max(probmap[5][2]))
                    plt.subplot(2, 2, 4)
                    plt.imshow(masked_images[5])
                    plt.savefig(PATH_TO_OUTPUTS+'/'+args.model_di_n+'/val_B_{:04d}.png'.format(epoch+start_epoch))
                    plt.close()
