...
from classifier import FaceDataset, Classifier, new_lms#, Classifier, ResNetLMS_L_v8
...
PATH_TO_MODELS = lp("/home/godsom/guesswhat/models/v8_3dot",clone=False)
PATH_TO_OUTPUTS = lp("/home/godsom/guesswhat/outputs/v8_3dot",clone=False)
...
def get_transform_mat(probmap, inv_std=inv_std):
    ...
    # masks = torch.zeros((probmap.shape[0],IMAGE_SIZE,IMAGE_SIZE,1))#.to(device)

    # masks = np.zeros((probmap.shape[0],IMAGE_SIZE,IMAGE_SIZE,1))
    # for i in range(masks.shape[0]):
    #     # masks[i,:,:,0] = create_blurred_circular_mask([IMAGE_SIZE,IMAGE_SIZE], 25, coors[i], sigma=15)
...
def train(cl_net, lm_net, train_dataloader, epochs, filename, checkpoint_frequency=checkpoint_frequency, val_dataloader=None):
    ...
    # if os.path.exists(lp("/home/godsom/guesswhat/logs/v8_3dot"+'/'+args.model_di_n,clone=False)):
        ...
        if not os.path.exists(lp("/home/godsom/guesswhat/logs/v8_3dot"+'/'+args.model_di_n,clone=False)):
            os.makedirs(lp("/home/godsom/guesswhat/logs/v8_3dot"+'/'+args.model_di_n,clone=False))
            os.makedirs(PATH_TO_MODELS+'/'+args.model_di_n)
        start_epoch = 0

    writer = SummaryWriter(lp("/home/godsom/guesswhat/logs/v8_3dot/"+args.model_di_n,clone=False))
    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            # running_loss += float(loss.item())
            cl_loss = 10*loss# + variance_loss(probmap, lms)#max(prob_sum-(IMAGE_SIZE*IMAGE_SIZE*ww), 0)# + variance_loss(probmap, lms)
            loss.backward()
            cl_optimizer.step()
...
def test():
    ...
    lm_net = ResNetLMS_L_v8(n_lm=N_KEYPOINT).to(device)
