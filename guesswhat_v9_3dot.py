...
file_dir =  lp('/home/godsom/Dataset/dot_6')
PATH_TO_MODELS = lp("/home/godsom/guesswhat/models/v9_3dot",clone=False)
PATH_TO_OUTPUTS = lp("/home/godsom/guesswhat/outputs/v9_3dot",clone=False)
...
def get_all_transform_mat():
    ...
    for i in range(prob_size):
        for j in range(prob_size):
            ...
            masks[i*prob_size+j] = get_gaussian_maps(torch.reshape(coor, (1, 1, 2)).to(device), inv_std)[0,:,:,0]
    # masks = torch.transpose(masks, 2, 3)
    # masks = torch.transpose(masks, 1, 2)

    return masks
all_masks = get_all_transform_mat().to(device)
...
def train(cl_net, lm_net, train_dataloader, epochs, filename, checkpoint_frequency=checkpoint_frequency, val_dataloader=None):
    ...
    if os.path.exists(PATH_TO_MODELS+'/'+args.model_di_n+f'/{args.model_di_n}.pt'):
        ...
        if not os.path.exists(lp("/home/godsom/guesswhat/logs/v9_3dot"+'/'+args.model_di_n,clone=False)):
            os.makedirs(lp("/home/godsom/guesswhat/logs/v9_3dot"+'/'+args.model_di_n,clone=False))
            os.makedirs(PATH_TO_MODELS+'/'+args.model_di_n)
        start_epoch = 0

    writer = SummaryWriter(lp("/home/godsom/guesswhat/logs/v9_3dot/"+args.model_di_n,clone=False))
    ...
    for epoch in range(epochs):
        ...
        for aa in range(30):
            ...
            r3 = 25#random.randint(40, 160)
            r4 = 100#random.randint(40, 160)
            ...
            surface.write_to_png("/home/godsom/Dataset/dot_6/{:03d}_{:01d}_0.png".format(aa,0))
            ####################################
            r1 = random.randint(40, 160)
            r2 = random.randint(40, 160)
            ...
            # write output
            surface.write_to_png("/home/godsom/Dataset/dot_6/{:03d}_{:01d}_0.png".format(aa,1))
            ####################################
            r1 = random.randint(40, 160)
            r2 = random.randint(40, 160)
            r3 = random.randint(40, 160)
            ...
            # write output
            surface.write_to_png("/home/godsom/Dataset/dot_6/{:03d}_{:01d}_0.png".format(aa,2))
            ####################################
            r1 = random.randint(40, 160)
            r2 = random.randint(40, 160)
            r3 = random.randint(40, 160)
            ...
            # write output
            surface.write_to_png("/home/godsom/Dataset/dot_6/{:03d}_{:01d}_0.png".format(aa,3))
            ...
            # write output
            surface.write_to_png("/home/godsom/Dataset/dot_6/{:03d}_{:01d}_0.png".format(aa,4))
            ...
            # write output
            surface.write_to_png("/home/godsom/Dataset/dot_6/{:03d}_{:01d}_0.png".format(aa,5))
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            if N_KEYPOINT==1:
                ...
                masked_images = tile(images, 0, 23*23) * tiled_masks
            ...
            tiled_labels = tile(labels, 0, 23*23)
            loss = torch.sum(criterion(outputs, tiled_labels) * torch.reshape(probmap, (1,23*23*batch_size))[0])
            prob_sum_m = torch.sum(torch.reshape(probmap, (probmap.shape[0],N_KEYPOINT,-1)),2)
            # print(prob_sum_m)
            prob_sum = torch.mean(prob_sum_m)
            neg_loss = 100*loss #+ variance_loss(probmap, lms)#max(prob_sum-(IMAGE_SIZE*IMAGE_SIZE*ww), 0)# + variance_loss(probmap, lms)
            ...
            # print(outputs)
            accu = sum(outputs == tiled_labels).item()/outputs.shape[0]
...
def validate(cl_net, lm_net, dataloader,epoch,inv_std_loop,start_epoch):
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            ...
            exact_match += sum(outputs == tiled_labels).item()
            if i==0:
                if N_KEYPOINT ==2:
                    probmap = probmap.cpu()
                    masked_images = torch.transpose(masked_images, 1 ,2)
                    ...
                    print(torch.reshape(probmap, (1,23*23*batch_size))[0,264])
                    print(torch.reshape(probmap, (1,23*23*batch_size))[0,163])
                    print(torch.reshape(probmap, (1,23*23*batch_size))[0,255])
                    ...
                    save_image(masked_images[1], PATH_TO_OUTPUTS+'/'+args.model_di_n+'/Bmaskedimg_{:04d}.png'.format(epoch+start_epoch))
...
def test():
    ...
    lm_net = ResNetLMS_L_v9(n_lm=N_KEYPOINT).to(device)
...
def train_save(checkpoint_frequency=checkpoint_frequency):
    training_loss, validation_loss = train(
        ...
        new_lms(n_lm=N_KEYPOINT).to(device),
