...
from classifier import FaceDataset, Classifier, new_lms#, Classifier, ResNetLMS_L_v9
# from resnet import ResNetLMS_L_v7
# from clms import ResNetLMS_L_v9
...
PATH_TO_MODELS = lp("/home/godsom/guesswhat/models/v9_dot",clone=False)
PATH_TO_OUTPUTS = lp("/home/godsom/guesswhat/outputs/v9_dot",clone=False)
...
def get_transform_mat(probmap, inv_std=inv_std):
    ...
    return coors, masks.cuda()

def get_all_transform_mat():
    # x = (torch.arange(0, prob_size, 1)*8.6+5).float()
    # x = 2*x/IMAGE_SIZE -1
    x = torch.linspace(-0.9, 0.9, steps=prob_size)
    # print(x)
    # assert False
    masks = torch.zeros((prob_size*prob_size, IMAGE_SIZE, IMAGE_SIZE))

    for i in range(prob_size):
        for j in range(prob_size):
            coor = torch.from_numpy(np.array([[x[i], x[j]]]))
            masks[i*prob_size+j] = get_gaussian_maps(torch.reshape(coor, (1, 1, 2)).cuda(), inv_std)[0,:,:,0]
    return masks
all_masks = get_all_transform_mat().cuda()
...
def get_dataloader(minibatch_size=batch_size, mode='train'):
    ...
    if mode == 'val':
        transformed_dataset = FaceDataset(root_dir, fl[180:2000], transforms.Compose(transforms_list))
...
def train(cl_net, lm_net, train_dataloader, epochs, filename, checkpoint_frequency=checkpoint_frequency, val_dataloader=None):
    ...
    if os.path.exists(PATH_TO_MODELS+'/'+args.model_di_n+f'/{args.model_di_n}.pt'):
        ...
        if not os.path.exists(lp("/home/godsom/guesswhat/logs/v9_dot"+'/'+args.model_di_n,clone=False)):
            os.makedirs(lp("/home/godsom/guesswhat/logs/v9_dot"+'/'+args.model_di_n,clone=False))
            os.makedirs(PATH_TO_MODELS+'/'+args.model_di_n)
        start_epoch = 0

    writer = SummaryWriter(lp("/home/godsom/guesswhat/logs/v9_dot/"+args.model_di_n,clone=False))
    ...
    for epoch in range(epochs):
        ...
        for aa in range(1000):
            ...
            # surface.write_to_png(lp("/home/godsom/Dataset/dot/{:03d}_{:01d}_0.png".format(aa,0), clone=False))
            surface.write_to_png("/home2/godsom/local_storage/home/godsom/Dataset/dot/{:03d}_{:01d}_0.png".format(aa,0))
            ####################################
            ...
            # write output
            # surface.write_to_png(lp("/home/godsom/Dataset/dot/{:03d}_{:01d}_0.png".format(aa,1), clone=False))
            surface.write_to_png("/home2/godsom/local_storage/home/godsom/Dataset/dot/{:03d}_{:01d}_0.png".format(aa,1))
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            images, labels, fn = batch['image'].cuda(), batch['label'].cuda(), batch['fn']
            ...
            if N_KEYPOINT==1:
                tiled_masks = all_masks[:,None].repeat(1,3,1,1).repeat(batch_size,1,1,1)
                masked_images = tile(images, 0, prob_size*prob_size) * tiled_masks
                # masked_images = images * probmap[:,0,:,:][:,None].repeat(1,3,1,1)
                # print(tiled_masks.shape)
                # print(all_masks[:,None].repeat(1,3,1,1).shape)
                # a = tiled_masks[264]
                ...
                comb_masks = torch.max(masks[:,0,:,:], masks[:,1,:,:])
                # comb_masks = torch.max(probmap[:,0,:,:], probmap[:,1,:,:])
                masked_images = images * comb_masks[:,None].repeat(1,3,1,1)
            elif N_KEYPOINT==4:
                comb_masks = torch.max(masks[:,0,:,:], masks[:,1,:,:])
                comb_masks = torch.max(comb_masks, masks[:,2,:,:])
                comb_masks = torch.max(comb_masks, masks[:,3,:,:])
                masked_images = images * comb_masks[:,None].repeat(1,3,1,1)
            ...
            tiled_labels = tile(labels, 0, prob_size*prob_size)
            loss = torch.sum(criterion(outputs, tiled_labels) * torch.reshape(probmap, (1,prob_size*prob_size*batch_size))[0])
            prob_sum_m = torch.sum(torch.reshape(probmap, (probmap.shape[0],N_KEYPOINT,-1)),2)
            # print(prob_sum_m)
            prob_sum = torch.mean(prob_sum_m)
            ps = torch.mean(torch.min(prob_sum_m - 10, 0)[0])
            neg_loss = 100*loss #- ps# + variance_loss(probmap, lms)
            # print(max(prob_sum-4, 0))
            # print(1-loss)x()
            ...
            # print(outputs)
            accu_nosum = outputs == tiled_labels
            accu = sum(accu_nosum).item()/outputs.shape[0]

        if (epoch) % 5 == 0 and val_dataloader is not None:
            print(torch.max(probmap[0]))
            print(prob_sum)
            # if inv_std_loop < 20:
            ...
            if N_KEYPOINT ==2:
                ...
                pm = torch.reshape(probmap, (batch_size,-1))
                max, inx = torch.max(pm, 1)
                pm_img = images * masks.repeat(1,3,1,1)
                save_image(pm_img[0], PATH_TO_OUTPUTS+'/'+args.model_di_n+'/train/hm_mask_{:04d}.png'.format(epoch+start_epoch))
                save_image(masked_images[inx[0]], PATH_TO_OUTPUTS+'/'+args.model_di_n+'/train/maskedimg_{:04d}.png'.format(epoch+start_epoch))
...
def validate(cl_net, lm_net, dataloader,epoch,inv_std_loop,start_epoch):
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            ...
            accu_nosum = outputs == tiled_labels
            exact_match += sum(accu_nosum).item()
            if i==0:
                if N_KEYPOINT ==2:
                    probmap = probmap.cpu()
                    masked_images = torch.transpose(masked_images, 1 ,2)
                    ...
                    plt.imshow(masked_images[1])
                    plt.subplot(2, 3, 3)
                    plt.imshow(masked_images[2])
                    plt.subplot(2, 3, 4)
                    plt.imshow(masked_images[3])
                    plt.subplot(2, 3, 5)
                    plt.imshow(masked_images[4])
                    plt.subplot(2, 3, 6)
                    plt.imshow(masked_images[5])
                    plt.savefig(PATH_TO_OUTPUTS+'/'+args.model_di_n+'/val_Z_{:04d}.png'.format(epoch+start_epoch))
                    plt.close()
                elif N_KEYPOINT ==4:
                    probmap = probmap.cpu()
                    masked_images = torch.transpose(masked_images, 1 ,2)
                    ...
                    # print(torch.reshape(probmap, (1,prob_size*prob_size*batch_size))[0,264])
                    # print(torch.reshape(probmap, (1,prob_size*prob_size*batch_size))[0,163])
                    # print(torch.reshape(probmap, (1,prob_size*prob_size*batch_size))[0,255])
                    # save_image(masked_images[255], PATH_TO_OUTPUTS+'/'+args.model_di_n+'/maskedimg_{:04d}.png'.format(epoch+start_epoch))
                    save_image(images[0], PATH_TO_OUTPUTS+'/'+args.model_di_n+'/img_{:04d}.png'.format(epoch+start_epoch))
                    save_image(probmap[0]/torch.max(probmap[0]), PATH_TO_OUTPUTS+'/'+args.model_di_n+'/hm_{:04d}.png'.format(epoch+start_epoch))

                    save_image(images[1], PATH_TO_OUTPUTS+'/'+args.model_di_n+'/Bimg_{:04d}.png'.format(epoch+start_epoch))
                    save_image(probmap[1]/torch.max(probmap[0]), PATH_TO_OUTPUTS+'/'+args.model_di_n+'/Bhm_{:04d}.png'.format(epoch+start_epoch))

                    pm = torch.reshape(probmap, (batch_size,-1))
                    # print(pm.shape)
                    max, inx = torch.max(pm, 1)
                    print('max_accu', accu_nosum[inx[0]])
                    # print(max)
                    pm_img = images * masks.repeat(1,3,1,1)
                    save_image(pm_img[0], PATH_TO_OUTPUTS+'/'+args.model_di_n+'/a_{:04d}.png'.format(epoch+start_epoch))
                    save_image(masked_images[inx[0]], PATH_TO_OUTPUTS+'/'+args.model_di_n+'/maskedimg_{:04d}.png'.format(epoch+start_epoch))
...
def test():
    ...
    lm_net = ResNetLMS_L_v9(n_lm=N_KEYPOINT).cuda()
        ...
        for i, batch in enumerate(get_dataloader(batch_size, 'test')):
            ...
            if i<50:
                if N_KEYPOINT ==2:
                    ...
                    plt.subplot(2, 3, 6)
                    plt.imshow(masked_images[5])
                    plt.savefig(PATH_TO_OUTPUTS+'/'+args.model_di_n+'/test/val_Z_{:04d}.png'.format(epoch+start_epoch))
                    plt.close()
                elif N_KEYPOINT ==4:
                    probmap = probmap.cpu()
                    masked_images = torch.transpose(masked_images, 1 ,2)
