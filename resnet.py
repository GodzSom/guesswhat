...
class ResNetC(nn.Module):
    def __init__(self, block = BasicBlock, num_blocks = [2,2,2,2], num_classes=2):
        ...
        self.linear1 = nn.Linear(64*100*100, 64)
...
class ResNetLMS_L(nn.Module):
    def __init__(self, block = BasicBlock, num_blocks = [2,2,2,2], n_lm = 2):
        super(ResNetLMS_L, self).__init__()
        self.in_planes = 64
        self.n_lm = n_lm
    ...
    def forward(self, x):
        ...
        # print(out.shape)
        out = out.view(out.shape[0], self.n_lm, 196*196)
        out = F.softmax(out, dim=2)
        out = out.view(out.shape[0], self.n_lm, 196, 196)
...
class ResNetLMS_L_v7(nn.Module):
    def __init__(self, block = BasicBlock, num_blocks = [2,2,2,2], n_lm = 2):
        super(ResNetLMS_L_v7, self).__init__()
        self.in_planes = 64
        self.n_lm = n_lm
